                ...
                <form action="{{ route('instructor.assignments.store', $course) }}" method="post">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="grading_type" class="form-select">
                            <option value="auto" {{ old('grading_type', 'manual') === 'auto' ? 'selected' : '' }}>Auto-grading</option>
                            <option value="manual" {{ old('grading_type', 'manual') === 'manual' ? 'selected' : '' }}>Manual grading by instructor</option>
                        </select>
                        <small class="text-muted">Auto-grading uses expected answers; manual grading requires instructor review.</small>
                    </div>
                    <div class="row">
                    ...
                    <div class="mb-3 form-check">
                        <input type="checkbox" name="allow_late_submission" value="1" class="form-check-input" id="allow_late_submission" {{ old('allow_late_submission', false) ? 'checked' : '' }}>
