<div class="discussion-reply-block">
    <div class="card mb-2 {{ $reply->is_instructor_answer ? 'border-success' : 'border-0' }} shadow-sm">
        <div class="card-body py-3">
            ...
            @foreach($reply->replies as $child)
                <div class="ms-4 mt-2">
                    @include('discussions.partials.reply', ['reply' => $child, 'discussion' => $discussion])
                </div>
            @endforeach
            @auth
                <div class="mt-2 ms-0">
                    @include('discussions.partials.reply-form', ['discussion' => $discussion, 'parentId' => $reply->id])
                </div>
            @endauth
        </div>
    </div>
